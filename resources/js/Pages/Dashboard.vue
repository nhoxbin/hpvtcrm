...
const regis = async (product, phone, methods) => {
    ...
    await axios.post(route('transactions.store'), {
        ...
        sendingRegis[product.id] = false;
        regisMsg[product.id] = response.data.message;
    });
}
                    ...
                    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                        <section class="max-w-4xl sm:px-2 lg:px-3">
                            <div class="grid grid-cols-1 md:grid-cols-2">
                                ...
                                <div :class="{'pointer-events-none': sendingRegis[product.id] && !transaction.hasOwnProperty(product.id)}" class="relative items-center block max-w-sm p-2 bg-white border border-gray-100 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700 m-2" v-for="(product, index) in workingData" :key="product.id">
                                    ...
                                    <p class="mb-3 font-normal text-gray-700 dark:text-gray-400"
                                        v-if="sendingRegis[product.id]">{{ phoneNumber[index] }}</p>
                                    ...
                                    <div v-if="!sendingRegis[product.id]">
                                        <div class="flex items-center justify-between">
                                        ...
                                        <div class="flex justify-center">
                                            <div class="mb-[0.125rem] mr-4 inline-block min-h-[1.5rem] pl-[1.5rem]"
                                                ...
                                                <input
                                                    ...
                                                    type="radio"
                                                    ref="regisMethod"
                                                    :id="'regisMethod'+method.name+product.id"
                                                    :name="'regisMethod'+product.id"
                                                    :value="method.name"
                                                    :checked="method.checked"
                                                    :disabled="method.disable" />
                                                <label class="mt-px inline-block pl-[0.15rem] hover:cursor-pointer" :for="'regisMethod'+method.name+product.id">{{ method.name }}</label>
                                            </div>
                                        </div>
                                    </div>
                                    <div v-if="sendingRegis[product.id] || (!sendingRegis[product.id] && regisMsg.hasOwnProperty(product.id))">{{ regisMsg[product.id] }}</div>
                                    <div v-if="sendingRegis[product.id] && !transaction.hasOwnProperty(product.id)" role="status" class="absolute -translate-x-1/2 -translate-y-1/2 top-2/4 left-1/2">
